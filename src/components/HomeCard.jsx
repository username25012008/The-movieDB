...
export default function HomeCard({ id, title, poster_path, release_date, vote_average }) {
    ...
    return (
        <div>
            <div className="card w-[155px] relative overflow-hidden flex-shrink-0">
                <img
                    ...
                    <div className="w-[35px] h-[35px] absolute top-[-20px] z-10 left-3 bg-darkblue rounded-full font-sans-new">
                        <Flat
                            ...
                            sx={{
                                ...
                                bgStrokeColor: vote_average > 30 && vote_average < 70 ? '#413C0F' : '#1f4529',
                                barWidth: 7,
                                valueColor: '#fff',
                ...
                <div className={`bl w-[155px] h-full absolute top-0 rounded-lg z-20 ${tripleDot == id ? '' : 'translate-y-full'} bg-gradient-to-t from-black/30 via-black/50 to-transparent backdrop-blur-lg`}></div>
            </div>
        </div>
    )
}
